...
#            print(json.dumps(job_info,indent=2))
            self.redisclient.set(f"jobs:{job_id}",json.dumps(job_info), ex = 60 * 60 * 24 *7)
            return job_id
    ...
    def update_job(self,job_id: str, update_job: Dict):
        try:
            ...
            self.redisclient.set(job_id,json.dumps(update_job), ex = 60 * 60 * 24 *7)
        except ValueError as e:
            print('i am here')
            traceback.print_exc()
